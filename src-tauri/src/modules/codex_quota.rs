...
fn build_exhausted_quota_snapshot_with_hint(
    ...
    let reset_after_seconds = reset_after_seconds.filter(|seconds| *seconds >= 0);
    let computed_reset_at =
        reset_at.or_else(|| reset_after_seconds.map(|seconds| now.saturating_add(seconds)));
    let hourly_reset_time =
        computed_reset_at.or_else(|| previous.and_then(|quota| quota.hourly_reset_time));
    let weekly_reset_time =
        computed_reset_at.or_else(|| previous.and_then(|quota| quota.weekly_reset_time));
    CodexQuota {
        hourly_percentage: 0,
        hourly_reset_time,
...
fn quota_fetch_error_has_reset_hint(message: &str) -> bool {
    extract_i64_marker_from_message(message, "[reset_at:").is_some()
        || extract_i64_marker_from_message(message, "[reset_after_seconds:").is_some()
}

fn count_present_quota_windows(quota: &CodexQuota) -> usize {
    let mut normalized = quota.clone();
    normalized.normalize_window_slots();

    let mut count = 0usize;
    if normalized.hourly_window_present.unwrap_or(true) {
        count += 1;
    }
    if normalized.weekly_window_present.unwrap_or(true) {
        count += 1;
    }
    count
}

fn should_replace_quota_with_exhausted_snapshot(account: &CodexAccount, message: &str) -> bool {
    let Some(existing_quota) = account.quota.as_ref() else {
        return true;
    };

    if quota_effective_remaining_percentage(existing_quota) == Some(0) {
        return true;
    }

    quota_fetch_error_has_reset_hint(message) && count_present_quota_windows(existing_quota) <= 1
}
...
fn write_quota_fetch_error(account: &mut CodexAccount, message: String) {
    ...
    if quota_exhausted {
        let message = account
            ...
            .unwrap_or_default();
        if should_replace_quota_with_exhausted_snapshot(account, message) {
...
fn paid_plan_without_subscription_expiry(account: &CodexAccount) -> bool {
    account
        .plan_type
        .as_deref()
        .is_some_and(|plan_type| codex_account::is_paid_codex_plan_type(Some(plan_type)))
        && account
            .subscription_active_until
            .as_deref()
            .and_then(parse_subscription_timestamp)
            .is_none()
}
...
fn normalize_subscription_retry_state(account: &mut CodexAccount) {
    if !subscription_missing_or_expired(account.subscription_active_until.as_deref())
        || paid_plan_without_subscription_expiry(account)
    {
...
fn should_attempt_subscription_refresh(
    ...
    if paid_plan_without_subscription_expiry(account) && !options.force {
        return false;
    }
...
async fn refresh_subscription_state(
    ...
    if subscription_missing_or_expired(account.subscription_active_until.as_deref()) {
        if paid_plan_without_subscription_expiry(account) {
            account.subscription_query_last_success_at = Some(now_timestamp());
            clear_subscription_retry_pending(account);
        } else {
            mark_subscription_retry_pending(
                account,
                Some("订阅接口未返回有效订阅时间".to_string()),
            );
        }
    } else {
        account.subscription_query_last_success_at = Some(now_timestamp());
...
fn direct_window_remaining_percentage(
...
fn apply_direct_window_to_quota_slots(
    ...
    let Some(remaining) = direct_window_remaining_percentage(window, force_exhausted) else {
        return;
    };
...
fn should_apply_direct_quota_observation(
    ...
    if observation.exhausted {
        ...
        if observation.quota.is_none() {
            return account
                .quota
                .as_ref()
                .and_then(quota_effective_remaining_percentage)
...
fn should_use_local_quota_short_circuit(options: RefreshQuotaOptions) -> bool {
    !options.force_live_refresh
}

fn build_subscription_refresh_options(options: RefreshQuotaOptions) -> SubscriptionRefreshOptions {
    SubscriptionRefreshOptions {
        force: options.force_subscription_refresh || options.force_live_refresh,
    }
}
...
async fn refresh_account_quota_once(
    ...
    if should_use_local_quota_short_circuit(options) {
    ...
    let subscription_options = build_subscription_refresh_options(options);
    ...
    let tasks: Vec<_> = accounts
        ...
        .map(|account| {
            ...
            async move {
                ...
                let result =
                    refresh_account_quota_with_options(&account_id, RefreshQuotaOptions::default())
                        .await;
...
mod tests {
    ...
    #[test]
    fn refresh_quota_options_default_to_non_forced_paths() {
        let options = RefreshQuotaOptions::default();

        assert!(should_use_local_quota_short_circuit(options));
        assert!(!build_subscription_refresh_options(options).force);
    }

    #[test]
    fn refresh_quota_options_force_live_refresh_bypasses_local_short_circuit() {
        let options = RefreshQuotaOptions {
    ...
    #[test]
    fn refresh_quota_options_force_subscription_refresh_keeps_local_short_circuit() {
        let options = RefreshQuotaOptions {
    ...
    #[test]
    fn paid_plan_without_subscription_expiry_does_not_retry_as_missing_subscription() {
        let mut account = test_account();
        account.plan_type = Some("plus".to_string());
        account.subscription_active_until = None;
        account.subscription_query_next_retry_at = Some(i64::MAX);
        account.subscription_query_last_error = Some("订阅接口未返回有效订阅时间".to_string());

        normalize_subscription_retry_state(&mut account);

        assert_eq!(account.subscription_query_next_retry_at, None);
        assert_eq!(account.subscription_query_last_error, None);
        assert!(!should_attempt_subscription_refresh(
            &account,
            SubscriptionRefreshOptions { force: false }
        ));
        assert!(should_attempt_subscription_refresh(
            &account,
            SubscriptionRefreshOptions { force: true }
        ));
    }

    #[test]
    fn paid_plan_expired_subscription_still_refreshes() {
        let mut account = test_account();
        account.plan_type = Some("plus".to_string());
        account.subscription_active_until = Some("1".to_string());
    ...
    #[test]
    fn quota_fetch_error_preserves_nonzero_dual_window_quota() {
        let mut account = test_account();
    ...
    #[test]
    fn quota_fetch_error_preserves_nonzero_dual_window_quota_with_reset_hint() {
        let mut account = test_account();
        account.quota = Some(CodexQuota {
            hourly_percentage: 64,
            hourly_reset_time: Some(111),
    ...
    #[test]
    fn quota_fetch_error_sets_initial_exhausted_quota_to_zero() {
        let mut account = test_account();
    ...
    #[test]
    fn exhausted_quota_snapshot_uses_reset_after_seconds_when_reset_at_is_missing() {
        let account = test_account();
        let now = 1_700_000_000;

        let quota = build_exhausted_quota_snapshot_with_hint(
            &account,
            "quota_refresh_error",
            None,
            Some(60),
        ...
        assert_eq!(quota.hourly_reset_time, Some(now + 60));
        assert_eq!(quota.weekly_reset_time, Some(now + 60));
        assert_eq!(
            quota
                .raw_data
                .as_ref()
    ...
    #[test]
    fn exhausted_quota_snapshot_ignores_negative_reset_after_seconds() {
        let mut account = test_account();
        account.quota = Some(CodexQuota {
            hourly_percentage: 64,
            hourly_reset_time: Some(111),
            ...
            weekly_window_minutes: Some(10080),
            weekly_window_present: Some(true),
            raw_data: Some(json!({ "quota_exhausted": true })),
        });
        let now = 1_700_000_000;
    ...
    #[test]
    fn quota_fetch_error_refreshes_existing_exhausted_snapshot_reset_hint() {
        let mut account = test_account();
        account.quota = Some(CodexQuota {
            hourly_percentage: 0,
            hourly_reset_time: Some(111),
    ...
    #[test]
    fn direct_codex_log_error_observation_maps_identity_without_overwriting_nonzero_quota() {
        let mut account = test_account();
        ...
        assert_eq!(quota.weekly_reset_time, Some(1_700_010_000));
    ...
    #[test]
    fn quota_usage_generic_limit_reached_preserves_plus_weekly_remaining_window() {
        let usage: UsageResponse = serde_json::from_str(
            r#"{
              "plan_type":"plus",
              "rate_limit":{
        ...
        assert_eq!(quota.hourly_reset_time, Some(1_700_001_800));
        assert_eq!(quota.hourly_window_present, Some(true));
        assert_eq!(quota.weekly_percentage, 69);
        assert_eq!(quota.weekly_window_minutes, Some(10_080));
        assert_eq!(quota.weekly_reset_time, Some(1_700_604_800));
        assert_eq!(quota.weekly_window_present, Some(true));
    }
    ...
    #[test]
    fn quota_usage_weekly_window_used_percent_zeroes_only_weekly_window() {
        let usage: UsageResponse = serde_json::from_str(
            r#"{
              "plan_type":"plus",
              "rate_limit":{
        ...
        assert_eq!(quota.hourly_percentage, 80);
        assert_eq!(quota.weekly_percentage, 0);
        assert_eq!(quota.weekly_reset_time, Some(1_700_604_800));
    }
    ...
    #[test]
    fn direct_session_generic_limit_reached_preserves_plus_weekly_remaining_window() {
        let line = serde_json::json!({
            "timestamp": "2026-05-25T11:42:31.809Z",
            "type": "event_msg",
            "payload": {
    ...
    #[test]
    fn direct_session_error_observation_preserves_nonzero_quota_snapshot() {
        let line = serde_json::json!({
            "timestamp": "2026-05-25T11:42:31Z",
            "type": "event_msg",
            "payload": {
        ...
        account.quota = Some(original_quota.clone());
        ...
        assert_eq!(
            serde_json::to_value(&account.quota).expect("quota should serialize"),
            serde_json::to_value(Some(original_quota)).expect("quota should serialize")
        );
        assert!(account.quota_error.is_none());
    }

    #[test]
    fn direct_session_error_observation_updates_existing_zero_snapshot_reset() {
        let line = serde_json::json!({
            "timestamp": "2026-05-25T11:42:31Z",
            "type": "event_msg",
            "payload": {
