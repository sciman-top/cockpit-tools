...
func BuildConfigChangeDetails(oldCfg, newCfg *config.Config) []string {
	...
	if !reflect.DeepEqual(oldCfg.TransportBypass, newCfg.TransportBypass) {
		changes = append(changes, fmt.Sprintf("transport-bypass: %d -> %d rules", len(oldCfg.TransportBypass), len(newCfg.TransportBypass)))
	}
