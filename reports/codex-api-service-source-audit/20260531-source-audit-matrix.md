...
审查日期：2026-05-31；2026-06-02 复核 OpenAI Codex 与本地参考源最新 HEAD
...
刷新命令均为非破坏性 `git fetch --prune` / `git pull --ff-only`。五个参考源均已刷新到远端当前快进状态，且工作树 clean。

2026-05-31 二次复核：官方 `openai-codex` 从 `966932124` 快进到 `cdde711fa`；该区间只修改 `codex-rs/app-server/src/request_processors/plugins.rs` 和 `codex-rs/app-server/tests/suite/v2/plugin_install.rs`，未触及 Responses/SSE/turn-state/`previous_response_id` 关键路径。
...
本轮深度比对后没有发现必须新增的 stream/failover 生产逻辑修复。2026-06-02 追加了更直白的 focused regression：`in_flight_stream_finishes_on_original_account_while_new_task_uses_replacement`，显式覆盖旧流已 `upstream_admitted` + `headers_written` + `first_chunk_written` 后，即使旧账号随后被记录为 `usage_limit_reached`，旧流仍在原账号自然完成，新独立请求切到替换账号。

本轮另有一项低风险稳定性优化：相同内容的 `auth.json` / `config.toml` 投影不再触发 atomic rewrite，避免 API 模式或账号投影重复保存时制造 `.bak`、mtime 抖动或 watcher 噪声。该优化不改变 stream 粘性语义，只降低 live/provider 周边扰动。
...
本轮报告/文档、投影稳定性、订阅状态和 continuity test timeout 改动使用非 live 验证：
...
5. `node scripts/release/preflight.cjs --skip-typecheck --skip-build --skip-cargo --skip-cargo-test`

本轮不执行真实 quota drain，按项目规则记为 live gate N/A：`reason=未获 AcknowledgeLiveUpstreamRisk 且真实 drain 会消耗账号池；alternative_verification=离线 audit fixture + Rust lib tests + release preflight；evidence_link=本报告与 test output；expires_at=下次明确授权 live drain 时`。

## Rollback

回滚方式：优先 `git revert <commit>`；若只回滚本轮未提交工作树，则还原本报告、`docs/reference-gateway-best-practices.md`、`src-tauri/src/modules/codex_account.rs`、`src-tauri/src/modules/codex_local_access.rs`、`src-tauri/src/modules/codex_quota.rs`、`src/pages/CodexAccountsPage.tsx`、`src/stores/useCodexAccountStore.ts`、`src/types/codex.ts`、`src/utils/installBrowserPreviewTauriStub.ts`。本轮没有修改 live config、账号/provider、release exe 或持久状态。
