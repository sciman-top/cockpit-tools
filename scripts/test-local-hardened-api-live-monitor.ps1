...
try {
  ...
  Write-AuditLines $auditPass @(
    ...
    [ordered]@{ schemaVersion = 1; timestamp = 5; requestId = "turn:sha256:current"; phase = "fallback_blocked"; route = "/v1/responses"; model = "gpt-5.5"; accountHash = "sha256:exhausted"; status = 429; errorType = "usage_limit_reached"; outcome = "hard_affinity"; detail = [ordered]@{ gateway_request_id = "gw-pass-1"; turn_lineage_id = "turn:sha256:current"; sticky_boundary = "codex_turn_state"; sticky_boundary_present = "true"; is_sticky_continuation = "true"; admission_stage = "upstream_admission_rejected"; admission_outcome = "terminal_quota_before_stream"; safe_account_switch = "false"; account_switch_block_reason = "hard_affinity_terminal_quota" } },
  ...
  Assert-Equal $passSummary.quotaContinuityVerdict.status "plus_like" "expected separate quota continuity verdict to identify plus-like behavior"
  Assert-Equal $passSummary.quotaContinuityVerdict.plusLike $true "expected plus-like verdict flag"
  Assert-Equal $passSummary.hostDriftVerdict.status "pass" "expected host drift verdict pass"
  ...
  Assert-Equal $currentTimeline.stickyBoundaries[0] "codex_turn_state" "expected current timeline sticky boundary"
  Assert-Equal $currentTimeline.hasStickyContinuation $true "expected sticky continuation flag for hard-affinity request"
  Assert-True ($currentTimeline.admissionOutcomes -contains "terminal_quota_before_stream") "expected current timeline admission attribution"
  Assert-True ($currentTimeline.safeAccountSwitches -contains "false") "expected current timeline safe-account-switch attribution"
...
"@ | Set-Content -LiteralPath $Path -Encoding UTF8
  ...
  Assert-Equal (($noNewRequestSummary.results | Where-Object name -eq "new_request_avoids_exhausted_account").status) "not_observed" "expected missing new request to be recorded without blocking"
  ...
  Assert-Equal $noNewRequestSummary.continuitySummary.newRequestAvoidsExhaustedCooldown.status "not_observed" "expected new request summary to be recorded without blocking"

  $dataRootTerminalNoNewRequest = Join-Path $tempRoot "data-terminal-no-new-request"
  $auditTerminalNoNewRequest = Join-Path $dataRootTerminalNoNewRequest "codex_local_access_audit.jsonl"
  Write-AuditLines $auditTerminalNoNewRequest @(
    [ordered]@{ schemaVersion = 1; timestamp = 1; requestId = "req-terminal"; phase = "listener"; route = "/v1/responses"; model = "gpt-5.5"; accountHash = "-"; outcome = "accepted"; detail = [ordered]@{ gateway_request_id = "gw-terminal-no-new" } },
    [ordered]@{ schemaVersion = 1; timestamp = 2; requestId = "req-terminal"; phase = "request_trace"; route = "/v1/responses"; model = "gpt-5.5"; accountHash = "-"; outcome = "prepared"; detail = [ordered]@{ gateway_request_id = "gw-terminal-no-new"; hard_affinity_continuity = "true"; sticky_boundary = "codex_turn_state" } },
    [ordered]@{ schemaVersion = 1; timestamp = 3; requestId = "req-terminal"; phase = "upstream_forward"; route = "/v1/responses"; model = "gpt-5.5"; accountHash = "sha256:exhausted"; status = 429; outcome = "response_received"; detail = [ordered]@{ gateway_request_id = "gw-terminal-no-new" } },
    [ordered]@{ schemaVersion = 1; timestamp = 4; requestId = "req-terminal"; phase = "classifier"; route = "/v1/responses"; model = "gpt-5.5"; accountHash = "sha256:exhausted"; status = 429; errorType = "usage_limit_reached"; outcome = "failover"; detail = [ordered]@{ gateway_request_id = "gw-terminal-no-new"; provider_code = "usage_limit_reached" } },
    [ordered]@{ schemaVersion = 1; timestamp = 5; requestId = "req-terminal"; phase = "model_cooldown_applied"; route = "/v1/responses"; model = "gpt-5.5"; accountHash = "sha256:exhausted"; status = 429; errorType = "usage_limit_reached"; outcome = "recorded"; detail = [ordered]@{ gateway_request_id = "gw-terminal-no-new" } },
    [ordered]@{ schemaVersion = 1; timestamp = 6; requestId = "req-terminal"; phase = "fallback_blocked"; route = "/v1/responses"; model = "gpt-5.5"; accountHash = "sha256:exhausted"; status = 429; errorType = "usage_limit_reached"; outcome = "hard_affinity"; detail = [ordered]@{ gateway_request_id = "gw-terminal-no-new"; admission_outcome = "terminal_quota_before_stream" } },
    [ordered]@{ schemaVersion = 1; timestamp = 7; requestId = "req-terminal"; phase = "final_response"; route = "/v1/responses"; model = "gpt-5.5"; accountHash = "sha256:exhausted"; status = 429; errorType = "usage_limit_reached"; outcome = "error"; detail = [ordered]@{ gateway_request_id = "gw-terminal-no-new"; admission_outcome = "terminal_quota_before_stream" } }
  )
  $terminalNoNewRequestOutput = & pwsh -NoProfile -ExecutionPolicy Bypass -File $monitorScript `
    -DurationSeconds 0 `
    -DataRoot $dataRootTerminalNoNewRequest `
    -CodexHome $codexHome `
    -CodexAppProcessNames "__cockpit_no_such_process__" `
    -IncludeExistingAudit `
    -RequireQuotaFallback `
  ...
  if ($LASTEXITCODE -ne 0) {
    throw "terminal-no-new-request fixture should not fail or block only because a new request was not sampled; exit_code=$LASTEXITCODE"
  }
  $terminalNoNewRequestSummary = Convert-JsonOutput $terminalNoNewRequestOutput "terminal-no-new-request fixture"
  Assert-Equal $terminalNoNewRequestSummary.overall "pass" "expected terminal quota without new-request sample to keep overall pass"
  Assert-Equal (($terminalNoNewRequestSummary.results | Where-Object name -eq "same_task_affinity_fallback_blocked").status) "pass" "expected structured terminal quota to pass"
  Assert-Equal (($terminalNoNewRequestSummary.results | Where-Object name -eq "new_request_avoids_exhausted_account").status) "not_observed" "expected missing new request sample not to block terminal quota evidence"
  Assert-Equal $terminalNoNewRequestSummary.continuitySummary.newRequestAvoidsExhaustedCooldown.status "not_observed" "expected continuity summary to mark missing new request as not observed"
  ...
  Write-AuditLines $auditMulti @(
    ...
    [ordered]@{ schemaVersion = 1; timestamp = 8; requestId = "req-a-next"; phase = "lease_granted"; route = "/v1/responses"; model = "gpt-5.5"; accountHash = "sha256:healthy-a"; outcome = "active"; detail = [ordered]@{ gateway_request_id = "gw-multi-a-next"; turn_lineage_id = "turn:sha256:multi-a-next" } },
    [ordered]@{ schemaVersion = 1; timestamp = 9; requestId = "req-a-next"; phase = "stream_write"; route = "/v1/responses"; model = "gpt-5.5"; accountHash = "sha256:healthy-a"; status = 200; streamState = "first_chunk_written"; outcome = "ok"; detail = [ordered]@{ gateway_request_id = "gw-multi-a-next"; turn_lineage_id = "turn:sha256:multi-a-next" } },
    ...
    [ordered]@{ schemaVersion = 1; timestamp = 18; requestId = "req-b-next"; phase = "lease_granted"; route = "/v1/responses"; model = "gpt-5.5"; accountHash = "sha256:healthy-b"; outcome = "active"; detail = [ordered]@{ gateway_request_id = "gw-multi-b-next"; turn_lineage_id = "turn:sha256:multi-b-next" } },
    [ordered]@{ schemaVersion = 1; timestamp = 19; requestId = "req-b-next"; phase = "stream_write"; route = "/v1/responses"; model = "gpt-5.5"; accountHash = "sha256:healthy-b"; status = 200; streamState = "first_chunk_written"; outcome = "ok"; detail = [ordered]@{ gateway_request_id = "gw-multi-b-next"; turn_lineage_id = "turn:sha256:multi-b-next" } },
  ...
  $dataRootSameTurnMultiGateway = Join-Path $tempRoot "data-same-turn-multi-gateway"
  $auditSameTurnMultiGateway = Join-Path $dataRootSameTurnMultiGateway "codex_local_access_audit.jsonl"
  Write-AuditLines $auditSameTurnMultiGateway @(
    [ordered]@{ schemaVersion = 1; timestamp = 1; requestId = "turn:sha256:same"; phase = "listener"; route = "/v1/responses"; model = "gpt-5.5"; accountHash = "-"; outcome = "accepted"; detail = [ordered]@{ gateway_request_id = "gw-same-open"; turn_lineage_id = "turn:sha256:same"; turn_lineage_source = "codex_turn_state" } },
    [ordered]@{ schemaVersion = 1; timestamp = 2; requestId = "turn:sha256:same"; phase = "request_trace"; route = "/v1/responses"; model = "gpt-5.5"; accountHash = "-"; outcome = "prepared"; detail = [ordered]@{ gateway_request_id = "gw-same-open"; turn_lineage_id = "turn:sha256:same"; hard_affinity_continuity = "true"; sticky_boundary = "x_codex_turn_state" } },
    [ordered]@{ schemaVersion = 1; timestamp = 3; requestId = "turn:sha256:same"; phase = "upstream_admitted"; route = "/v1/responses"; model = "gpt-5.5"; accountHash = "sha256:exhausted"; status = 200; outcome = "admitted"; detail = [ordered]@{ gateway_request_id = "gw-same-open"; turn_lineage_id = "turn:sha256:same" } },
    [ordered]@{ schemaVersion = 1; timestamp = 4; requestId = "turn:sha256:same"; phase = "lease_granted"; route = "/v1/responses"; model = "gpt-5.5"; accountHash = "sha256:exhausted"; outcome = "active"; detail = [ordered]@{ gateway_request_id = "gw-same-open"; turn_lineage_id = "turn:sha256:same" } },
    [ordered]@{ schemaVersion = 1; timestamp = 5; requestId = "turn:sha256:same"; phase = "stream_write"; route = "/v1/responses"; model = "gpt-5.5"; accountHash = "sha256:exhausted"; status = 200; streamState = "first_chunk_written"; outcome = "ok"; detail = [ordered]@{ gateway_request_id = "gw-same-open"; turn_lineage_id = "turn:sha256:same" } },
    [ordered]@{ schemaVersion = 1; timestamp = 6; requestId = "turn:sha256:same"; phase = "listener"; route = "/v1/responses"; model = "gpt-5.5"; accountHash = "-"; outcome = "accepted"; detail = [ordered]@{ gateway_request_id = "gw-same-quota"; turn_lineage_id = "turn:sha256:same"; turn_lineage_source = "codex_turn_state" } },
    [ordered]@{ schemaVersion = 1; timestamp = 7; requestId = "turn:sha256:same"; phase = "request_trace"; route = "/v1/responses"; model = "gpt-5.5"; accountHash = "-"; outcome = "prepared"; detail = [ordered]@{ gateway_request_id = "gw-same-quota"; turn_lineage_id = "turn:sha256:same"; hard_affinity_continuity = "true"; sticky_boundary = "x_codex_turn_state" } },
    [ordered]@{ schemaVersion = 1; timestamp = 8; requestId = "turn:sha256:same"; phase = "upstream_forward"; route = "/v1/responses"; model = "gpt-5.5"; accountHash = "sha256:exhausted"; status = 429; outcome = "response_received"; detail = [ordered]@{ gateway_request_id = "gw-same-quota"; turn_lineage_id = "turn:sha256:same"; retry_after_ms = "3600000" } },
    [ordered]@{ schemaVersion = 1; timestamp = 9; requestId = "turn:sha256:same"; phase = "quota_classification"; route = "/v1/responses"; model = "gpt-5.5"; accountHash = "sha256:exhausted"; status = 429; errorType = "usage_limit_reached"; outcome = "exhausted"; detail = [ordered]@{ gateway_request_id = "gw-same-quota"; turn_lineage_id = "turn:sha256:same"; provider_code = "usage_limit_reached" } },
    [ordered]@{ schemaVersion = 1; timestamp = 10; requestId = "turn:sha256:same"; phase = "model_cooldown_applied"; route = "/v1/responses"; model = "gpt-5.5"; accountHash = "sha256:exhausted"; status = 429; errorType = "usage_limit_reached"; outcome = "recorded"; detail = [ordered]@{ gateway_request_id = "gw-same-quota"; turn_lineage_id = "turn:sha256:same" } },
    [ordered]@{ schemaVersion = 1; timestamp = 11; requestId = "turn:sha256:same"; phase = "fallback_blocked"; route = "/v1/responses"; model = "gpt-5.5"; accountHash = "sha256:exhausted"; status = 429; errorType = "usage_limit_reached"; outcome = "hard_affinity"; detail = [ordered]@{ gateway_request_id = "gw-same-quota"; turn_lineage_id = "turn:sha256:same" } },
    [ordered]@{ schemaVersion = 1; timestamp = 12; requestId = "turn:sha256:same"; phase = "final_response"; route = "/v1/responses"; model = "gpt-5.5"; accountHash = "sha256:exhausted"; status = 429; errorType = "usage_limit_reached"; outcome = "error"; detail = [ordered]@{ gateway_request_id = "gw-same-quota"; turn_lineage_id = "turn:sha256:same" } },
    [ordered]@{ schemaVersion = 1; timestamp = 13; requestId = "turn:sha256:same"; phase = "stream_write"; route = "/v1/responses"; model = "gpt-5.5"; accountHash = "sha256:exhausted"; status = 200; streamState = "finished"; outcome = "ok"; detail = [ordered]@{ gateway_request_id = "gw-same-open"; turn_lineage_id = "turn:sha256:same" } },
    [ordered]@{ schemaVersion = 1; timestamp = 14; requestId = "turn:sha256:same"; phase = "stream_completed"; route = "/v1/responses"; model = "gpt-5.5"; accountHash = "sha256:exhausted"; outcome = "completed"; detail = [ordered]@{ gateway_request_id = "gw-same-open"; turn_lineage_id = "turn:sha256:same"; upstream_response_id_hash = "response:sha256:same-open" } },
    [ordered]@{ schemaVersion = 1; timestamp = 15; requestId = "turn:sha256:same"; phase = "lease_released"; route = "/v1/responses"; model = "gpt-5.5"; accountHash = "sha256:exhausted"; outcome = "completed"; detail = [ordered]@{ gateway_request_id = "gw-same-open"; turn_lineage_id = "turn:sha256:same" } },
    [ordered]@{ schemaVersion = 1; timestamp = 16; requestId = "turn:sha256:next-after-same"; phase = "listener"; route = "/v1/responses"; model = "gpt-5.5"; accountHash = "-"; outcome = "accepted"; detail = [ordered]@{ gateway_request_id = "gw-next-after-same"; turn_lineage_id = "turn:sha256:next-after-same"; turn_lineage_source = "codex_turn_state" } },
    [ordered]@{ schemaVersion = 1; timestamp = 17; requestId = "turn:sha256:next-after-same"; phase = "request_trace"; route = "/v1/responses"; model = "gpt-5.5"; accountHash = "-"; outcome = "prepared"; detail = [ordered]@{ gateway_request_id = "gw-next-after-same"; turn_lineage_id = "turn:sha256:next-after-same"; hard_affinity_continuity = "false" } },
    [ordered]@{ schemaVersion = 1; timestamp = 18; requestId = "turn:sha256:next-after-same"; phase = "routing_decision"; route = "/v1/responses"; model = "gpt-5.5"; accountHash = "sha256:healthy"; status = 200; outcome = "selected"; detail = [ordered]@{ gateway_request_id = "gw-next-after-same"; turn_lineage_id = "turn:sha256:next-after-same" } },
    [ordered]@{ schemaVersion = 1; timestamp = 19; requestId = "turn:sha256:next-after-same"; phase = "lease_granted"; route = "/v1/responses"; model = "gpt-5.5"; accountHash = "sha256:healthy"; outcome = "active"; detail = [ordered]@{ gateway_request_id = "gw-next-after-same"; turn_lineage_id = "turn:sha256:next-after-same" } },
    [ordered]@{ schemaVersion = 1; timestamp = 20; requestId = "turn:sha256:next-after-same"; phase = "stream_write"; route = "/v1/responses"; model = "gpt-5.5"; accountHash = "sha256:healthy"; status = 200; streamState = "first_chunk_written"; outcome = "ok"; detail = [ordered]@{ gateway_request_id = "gw-next-after-same"; turn_lineage_id = "turn:sha256:next-after-same" } },
    [ordered]@{ schemaVersion = 1; timestamp = 21; requestId = "turn:sha256:next-after-same"; phase = "stream_completed"; route = "/v1/responses"; model = "gpt-5.5"; accountHash = "sha256:healthy"; outcome = "completed"; detail = [ordered]@{ gateway_request_id = "gw-next-after-same"; turn_lineage_id = "turn:sha256:next-after-same"; upstream_response_id_hash = "response:sha256:next" } }
  )
  $sameTurnMultiGatewayOutput = & pwsh -NoProfile -ExecutionPolicy Bypass -File $monitorScript `
    -DurationSeconds 0 `
    -DataRoot $dataRootSameTurnMultiGateway `
    -CodexHome $codexHome `
    -CodexAppProcessNames "__cockpit_no_such_process__" `
    -IncludeExistingAudit `
    -RequireQuotaFallback `
  ...
  if ($LASTEXITCODE -ne 0) {
    throw "same-turn multi-gateway fixture failed with exit_code=$LASTEXITCODE"
  }
  $sameTurnMultiGatewaySummary = Convert-JsonOutput $sameTurnMultiGatewayOutput "same-turn multi-gateway fixture"
  Assert-Equal $sameTurnMultiGatewaySummary.overall "pass" "expected same-turn multi-gateway fixture overall pass"
  Assert-Equal $sameTurnMultiGatewaySummary.audit.sameTaskAffinityFallbackBlockedCount 1 "expected same turn hard-affinity block"
  Assert-Equal $sameTurnMultiGatewaySummary.audit.sameTaskAffinityStructuredQuotaTerminal429Count 1 "expected structured hard-affinity quota terminal"
  Assert-Equal $sameTurnMultiGatewaySummary.audit.interruptedStreamCount 0 "pre-stream cooldown on a later gateway request must not interrupt an earlier stream"
  Assert-Equal $sameTurnMultiGatewaySummary.audit.terminalErrorStreamCount 0 "pre-stream final 429 must not be attributed to an earlier stream"
  Assert-Equal $sameTurnMultiGatewaySummary.audit.openStreamCount 0 "upstream_admitted and lease_granted with the same gateway id must merge into one completed stream"
  Assert-Equal (($sameTurnMultiGatewaySummary.results | Where-Object name -eq "accepted_stream_continuity").status) "pass" "expected same-turn multi-gateway stream continuity pass"
  Assert-Equal (($sameTurnMultiGatewaySummary.results | Where-Object name -eq "accepted_headers_first_chunk_continuity").status) "pass" "expected explicit headers/first-chunk continuity result pass"
  Assert-Equal $sameTurnMultiGatewaySummary.continuitySummary.acceptedHeadersFirstChunkContinuity.status "pass" "expected explicit headers/first-chunk continuity summary pass"
  Assert-True ($null -ne ($sameTurnMultiGatewaySummary.audit.streamSummaries | Where-Object gatewayRequestId -eq "gw-same-open")) "expected open stream summary to keep gateway id"
  ...
  Write-AuditLines $auditAccountExhaustionInFlight @(
    ...
    [ordered]@{ schemaVersion = 1; timestamp = 4; requestId = "turn:sha256:inflight"; phase = "upstream_admitted"; route = "/v1/responses"; model = "gpt-5.5"; accountHash = "sha256:old"; status = 200; outcome = "accepted"; detail = [ordered]@{ gateway_request_id = "gw-inflight"; turn_lineage_id = "turn:sha256:inflight"; upstream_response_status = "200" } },
    [ordered]@{ schemaVersion = 1; timestamp = 5; requestId = "turn:sha256:inflight"; phase = "stream_write"; route = "/v1/responses"; model = "gpt-5.5"; accountHash = "sha256:old"; status = 200; streamState = "headers_written"; outcome = "ok"; detail = [ordered]@{ gateway_request_id = "gw-inflight"; turn_lineage_id = "turn:sha256:inflight" } },
    [ordered]@{ schemaVersion = 1; timestamp = 6; requestId = "turn:sha256:inflight"; phase = "stream_write"; route = "/v1/responses"; model = "gpt-5.5"; accountHash = "sha256:old"; status = 200; streamState = "first_chunk_written"; outcome = "ok"; detail = [ordered]@{ gateway_request_id = "gw-inflight"; turn_lineage_id = "turn:sha256:inflight" } },
    [ordered]@{ schemaVersion = 1; timestamp = 7; requestId = "turn:sha256:exhaust-trigger"; phase = "upstream_forward"; route = "/v1/responses"; model = "gpt-5.5"; accountHash = "sha256:old"; status = 429; errorType = "usage_limit_reached"; outcome = "response_received"; detail = [ordered]@{ gateway_request_id = "gw-exhaust-trigger"; turn_lineage_id = "turn:sha256:exhaust-trigger"; provider_code = "usage_limit_reached" } },
    [ordered]@{ schemaVersion = 1; timestamp = 8; requestId = "turn:sha256:exhaust-trigger"; phase = "quota_classification"; route = "/v1/responses"; model = "gpt-5.5"; accountHash = "sha256:old"; status = 429; errorType = "usage_limit_reached"; outcome = "classified"; detail = [ordered]@{ gateway_request_id = "gw-exhaust-trigger"; turn_lineage_id = "turn:sha256:exhaust-trigger"; provider_code = "usage_limit_reached" } },
    [ordered]@{ schemaVersion = 1; timestamp = 9; requestId = "turn:sha256:exhaust-trigger"; phase = "model_cooldown_applied"; route = "/v1/responses"; model = "gpt-5.5"; accountHash = "sha256:old"; status = 429; errorType = "usage_limit_reached"; outcome = "recorded"; detail = [ordered]@{ gateway_request_id = "gw-exhaust-trigger"; turn_lineage_id = "turn:sha256:exhaust-trigger" } },
    [ordered]@{ schemaVersion = 1; timestamp = 10; requestId = "turn:sha256:inflight"; phase = "stream_write"; route = "/v1/responses"; model = "gpt-5.5"; accountHash = "sha256:old"; status = 200; streamState = "finished"; outcome = "ok"; detail = [ordered]@{ gateway_request_id = "gw-inflight"; turn_lineage_id = "turn:sha256:inflight" } },
    [ordered]@{ schemaVersion = 1; timestamp = 11; requestId = "turn:sha256:inflight"; phase = "stream_completed"; route = "/v1/responses"; model = "gpt-5.5"; accountHash = "sha256:old"; outcome = "completed"; detail = [ordered]@{ gateway_request_id = "gw-inflight"; turn_lineage_id = "turn:sha256:inflight"; lease_id = "inflight-1"; upstream_response_id_hash = "response:sha256:inflight" } }
  )
  ...
  Assert-Equal $accountExhaustionInFlightSummary.audit.upstreamAdmittedInFlightAtAccountExhaustionCount 1 "expected one upstream-admitted stream at exhaustion"
  Assert-Equal $accountExhaustionInFlightSummary.audit.headersWrittenInFlightAtAccountExhaustionCount 1 "expected one headers-written stream at exhaustion"
  Assert-Equal $accountExhaustionInFlightSummary.audit.firstChunkInFlightAtAccountExhaustionCount 1 "expected one first-chunk stream at exhaustion"
  Assert-Equal $accountExhaustionInFlightSummary.audit.admittedHeadersFirstChunkInFlightAtAccountExhaustionCount 1 "expected one admitted headers first-chunk stream at exhaustion"
  ...
  Assert-Equal $accountExhaustionInFlightSummary.audit.completedAfterFirstChunkAtAccountExhaustionCount 1 "expected first-chunk stream to complete after exhaustion"
  Assert-Equal $accountExhaustionInFlightSummary.audit.completedAfterAdmittedHeadersFirstChunkAtAccountExhaustionCount 1 "expected admitted first-chunk stream to complete after exhaustion"
  ...
  Assert-Equal (($accountExhaustionInFlightSummary.results | Where-Object name -eq "first_chunk_streams_survive_account_exhaustion").status) "pass" "expected first-chunk account exhaustion stream continuity result pass"
  Assert-Equal (($accountExhaustionInFlightSummary.results | Where-Object name -eq "admitted_first_chunk_streams_survive_account_exhaustion").status) "pass" "expected admitted first-chunk account exhaustion stream continuity result pass"
  ...
  Assert-Equal $accountExhaustionInFlightSummary.continuitySummary.firstChunkInFlightContinuity.status "pass" "expected first-chunk continuity summary for account exhaustion stream pass"
  Assert-Equal $accountExhaustionInFlightSummary.continuitySummary.admittedFirstChunkInFlightContinuity.status "pass" "expected admitted first-chunk continuity summary for account exhaustion stream pass"
