...
- Codex-facing `/v1/responses` 是例外：`429 retry-limit`、静默 SSE、transport `503/pool_unavailable`、`response.failed`、heartbeat-only open wait 都会破坏当前 Codex turn。新 admission 全池不可调度时，只允许短等待恢复，且等待必须落在本次请求超时预算内；若不能在短等待内恢复，必须返回 `200` completed Responses 形态：streaming 使用完整 `response.created -> ... -> response.completed -> [DONE]`，non-stream 使用 `status=completed` JSON，并在 assistant text 中说明 `Cockpit API Service pool_unavailable` 与 `recover_action`。该本地响应不打上游、不释放无关 active stream，也不得生成空白 assistant message 伪装成正常完成；若它出现在同任务 hard-affinity block 之后，只能作为提前结束风险和监测失败证据。
- monitor 必须把 transport 503、`response.failed`、旧 `outcome=in_band_synthetic`、heartbeat/open/parked pool_wait、SSE idle，以及 `stream disconnected before completion: Cockpit API Service pool_unavailable` 标成连续性回归；新 admission 全池耗尽时的 `streamState=completed` / `outcome=in_band_local_completion` 是显式本地闭合证据，但同任务 hard-affinity block 后出现该闭合必须判为提前结束风险。
...
- Codex turn hard-affinity 只使用官方 sticky-routing token `x-codex-turn-state`。`x-codex-turn-metadata` / `turn_id` 只能作为脱敏 audit lineage，不得阻止 fallback；`x-client-request-id` 是 thread 级观测字段，不能把同一 thread 的新 turn 误判为旧任务。
...
单号池通过后，再放入 2-3 个账号，保持 `maxRetryAccounts >= 2`，验证 selector/sticky/health 不乱轮换：带 `x-codex-turn-state` 的同任务 hard-affinity 429 必须被 `fallback_blocked` 拦住并等待真实 terminal 或进入明确失败证据；只有 `x-codex-turn-metadata` 的请求不得触发 hard-affinity，新的独立请求应避开 cooldown/exhausted 账号并选择健康账号。
...
同任务真实 upstream terminal completion，并观察到后续新请求命中健康账号；如果第一个账号没有返回 429，
或同任务被本地 `in_band_local_completion` 闭合，报告会标为 `blocked/fail`，不能当作额度耗尽不中断验收通过。
...
该命令适合验证“手动号池中的某个账号真实 429 后，同任务是否仍粘原账号并跑到真实 terminal，同时新独立请求是否切到健康账号”。
如果手动号池只能产生 `429 -> fallback_blocked(hard_affinity) -> in_band_local_completion`，说明旧任务被本地 completed 提前闭合；脚本会判为失败，不能当作额度耗尽不中断验收通过。
...
- 通过 `localStorage` 注入 `agtools.codex.accounts.cache`、`agtools.codex.accounts.current`、
  `agtools.codex.local_access.state.preview`、`agtools.codex.wakeup.overview.preview`
- 由浏览器自动导航到 `codex-api-service` / `codex` 页面，核对默认文案、health panel、
  selector reason、blocked reason、recover action、首页 inline card explainability、
  wakeup/reset 风险提示和 DOM 脱敏状态

当前证据见
`reports/local-hardened-api-smoke/browser-preview-ui-smoke-20260607.md` 和
...
- API 服务首页 inline card 直接可见 `最近调度`、`当前阻断`、`尝试上限`、`恢复动作`、`建议等待`
- Codex wakeup 任务卡、quota reset 低频提示、reset window 说明、startup 延迟预览和完整邮箱/title 脱敏

该路径只替代低风险 preview/default UI 验收，不替代 live Tauri、tray、
...
- 同任务 `429 usage_limit_reached -> model_cooldown_applied -> fallback_blocked(hard_affinity)` 后是否真实 upstream terminal，而不是被本地 `in_band_local_completion` 提前闭合
