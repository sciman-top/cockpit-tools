...
| NPB-03 | 性能基线报告 | 已补首份 isolated synthetic `M/L` 基线与 browser-preview modal 首开 app-safe 样本；仍缺更接近真实桌面运行态的交互样本和阶段性复测 | `reports/` 下的启动、轮询、切换、刷新、大号池排序基线与复测报告 |
...
| NPB-06 | UI smoke 自动化 | preview/default 与 wakeup/reset UI 已由 browser-preview 覆盖；2026-06-07 已补 live UI smoke 分层地图与只读现场基线 | 剩余仍是高风险 live Tauri/tray/系统通知/continuity 提示自动化，需要在合适的 live 运行场景下继续取证 |
