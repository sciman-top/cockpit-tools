...
export function CodexLocalAccessModal({
  ...
  const [memberRefreshBusy, setMemberRefreshBusy] = useState(false);
  ...
  const selectorReasonLabel = useMemo(() => {
    return getCodexLocalAccessSelectorReasonLabel(selectorInsight?.selectedReason, t);
  }, [selectorInsight?.selectedReason, t]);
  const selectorSkippedSummary = useMemo(() => {
    return buildCodexLocalAccessSkippedReasonSummary(
      selectorInsight?.skippedCountsByReason,
      t,
      formatCompactNumber,
    );
  }, [selectorInsight?.skippedCountsByReason, t]);
  const blockedRecoverActionLabel = useMemo(() => {
    return getCodexLocalAccessBlockedRecoverActionLabel(
      blockedInsight?.recoverAction,
      t,
    );
  ...
  const serviceAccounts = useMemo(() => accounts, [accounts]);
  ...
  const visibleAccounts = useMemo(() => {
    ...
    const selectedTypeValues = Array.from(selectedTypes);
    ...
    return sorted.filter((account) => {
      ...
      if (selectedGroups.size > 0) {
        ...
        for (const id of accountGroupIds) {
      ...
      const accountHealth = accountHealthById.get(account.id);
      const isBlockedByIssue =
        requireValidAccounts || selectedTypes.has('ERROR')
          ? isLocalAccessSelectionBlockedByIssue(account, accountHealth)
          : false;
      ...
      if (selectedTypes.size > 0) {
        ...
        const matchesType = selectedTypeValues.some((type) => {
      ...
      if (queryText) {
        ...
        if (!displayName.includes(queryText) && !groupNames.includes(queryText)) {
          return false;
        }
      }
  ...
  const resolveStoredAccountIssueMeta = (
    ...
    if (issueInfo.kind === 'cooldown') {
      return {
        badge: t('codex.quotaError.cooldownBadge', '请求冷却'),
        detail: t('codex.quotaError.cooldownDetail', {
  ...
  const customRoutingAccounts = useMemo(() => {
    ...
    return currentIds
      .map((accountId) => accountById.get(accountId))
      .filter((account): account is CodexAccount => Boolean(account));
  }, [accountById, collection?.accountIds]);
  ...
  const customRoutingTierCounts = useMemo(() => {
    ...
    customRoutingAccounts.forEach((account) => {
      const hasBlockingError = isBlockingCodexQuotaError(account.quota_error);
      if (!hasBlockingError) {
        counts.VALID += 1;
      }
      const tier = getCodexPlanFilterKey(account);
  ...
  const customRoutingTierFilterOptions = useMemo<MultiSelectFilterOption[]>(
    () => [
      ...
      },
      {
        value: 'API_KEY',
        label: formatQuotaPoolLabel(
          `API Key (${customRoutingTierCounts.API_KEY})`,
          customRoutingQuotaPoolSummary.byPlan.API_KEY,
          quotaPoolLabels.hourly,
          quotaPoolLabels.weekly,
        ),
      },
  ...
  const moveMember = (accountId: string, direction: -1 | 1) => {
    if (actionBusy) return;
    const currentIndex = selectedOrderForSave.indexOf(accountId);
    if (currentIndex < 0) return;
    const nextIndex = currentIndex + direction;
    if (nextIndex < 0 || nextIndex >= selectedOrderForSave.length) return;
    const nextOrder = [...selectedOrderForSave];
  ...
  const handleRefreshSelectedMembers = async () => {
    if (memberRemovalSelected.size === 0 || actionBusy || memberRefreshBusy) return;
    setError('');
    setNotice('');
  ...
  const handleChangeRuntimeMode = async (nextMode: string) => {
    const isModeChange = nextMode !== selectedRuntimeMode;
    const needsProjectionRepair =
      !isModeChange &&
      runtimeMode?.mode === nextMode &&
      (runtimeMode.accountKind === 'unknown' || !runtimeMode.currentAccountId);
    if (!isModeChange && !needsProjectionRepair) return;

    const isDirectProjection = nextMode === 'direct_projection';
    if (isDirectProjection && isModeChange) {
                      ...
                      {selectorInsight && (
                        <div className="codex-local-access-health-insight-card">
                          ...
                          <div className="codex-local-access-health-insight-meta">
                            ...
                            {selectorInsight.capApplied && (
                              <span>
                                {t('codex.localAccess.health.selectorCap', {
                                  count: selectorInsight.capLimit,
                    ...
                    <div className="codex-local-access-current-member-actions">
                      ...
                      >
                        <RefreshCw
                          size={14}
                          className={
                            refreshing || memberRefreshBusy ? 'loading-spinner' : ''
                          }
                        />
                        <span>
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className="codex-local-access-member-inline-actions">
                            <button
                              type="button"
                              className="folder-icon-btn codex-local-access-member-icon-btn codex-local-access-move-member-btn"
                              onClick={() => moveMember(account.id, -1)}
                              disabled={actionBusy || index === 0}
                              title={t('codex.localAccess.moveMemberUp', '上移账号')}
                              aria-label={t('codex.localAccess.moveMemberUp', '上移账号')}
                            >
                              <ChevronUp size={14} />
                            </button>
                            <button
                              ...
                              className="folder-icon-btn codex-local-access-member-icon-btn codex-local-access-move-member-btn"
                              onClick={() => moveMember(account.id, 1)}
                              disabled={actionBusy || index === selectedMemberAccounts.length - 1}
                              title={t('codex.localAccess.moveMemberDown', '下移账号')}
                              aria-label={t('codex.localAccess.moveMemberDown', '下移账号')}
                            >
                              <ChevronDown size={14} />
                            </button>
